...
describe('MinHeap', () => {
  it('pops elements in ascending order', async () => {
...
describe('VirtualClock', () => {
  ...
  it('starts at the given time', async () => {
    // Expected: 0
    expect(clock.now()).toBe(0);
  });

  it('advance() moves time forward', async () => {
    await clock.advance(100);
    // Expected: 100
    expect(clock.now()).toBe(100);
  });

  it('fires timers in scheduled order', async () => {
    const order: number[] = [];
  ...
  it('FIFO for same-time timers', async () => {
    const order: string[] = [];
  ...
  it('handles timers that schedule timers within the same window', async () => {
    const order: number[] = [];
    clock.setTimeout(() => {
      order.push(1);
  ...
  it('clearInterval stops a repeating timer', async () => {
    const ticks: number[] = [];
  ...
  it('freeze() prevents advancement', async () => {
    clock.freeze();
  ...
  it('pending() lists all scheduled timers', async () => {
  ...
  it('advanceTo() jumps to an exact timestamp', async () => {
    const fired: number[] = [];
...
describe('install / uninstall', () => {
  it('patches Date.now() and performance.now()', async () => {
