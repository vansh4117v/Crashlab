...
```
Simulation.run({ seeds: N })
│
├─ _startMongo()          → MongoMemoryServer (one process per run)
│
└─ for each seed × scenario
   └─ Worker thread (isolated globals)
      ├─ createEnv(seed)  → fresh VirtualClock, PRNG, Scheduler, all mocks
      ├─ install patches  → Date.now, Math.random, net.createConnection, fetch
      ├─ import(scenario) → dynamic ES module load (file-based)
      │  or vm.runInContext(fnSource) (inline legacy)
